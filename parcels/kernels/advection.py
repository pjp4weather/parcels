...
def AdvectionRK4(particle, fieldset, time, dt):
    ...
    (u2, v2) = fieldset.UV[time + .5 * dt, particle.depth, lat1, lon1]
    lon2, lat2 = (particle.lon + u2*.5*dt, particle.lat + v2*.5*dt)
    (u3, v3) = fieldset.UV[time + .5 * dt, particle.depth, lat2, lon2]
    lon3, lat3 = (particle.lon + u3*dt, particle.lat + v3*dt)
    (u4, v4) = fieldset.UV[time + dt, particle.depth, lat3, lon3]
...
def AdvectionRK4_3D(particle, fieldset, time, dt):
    ...
    (u2, v2) = fieldset.UV[time + .5 * dt, dep1, lat1, lon1]
    w2 = fieldset.W[time + .5 * dt, dep1, lat1, lon1]
    lon2 = particle.lon + u2*.5*dt
    lat2 = particle.lat + v2*.5*dt
    dep2 = particle.depth + w2*.5*dt
    (u3, v3) = fieldset.UV[time + .5 * dt, dep2, lat2, lon2]
    w3 = fieldset.W[time + .5 * dt, dep2, lat2, lon2]
    lon3 = particle.lon + u3*dt
    lat3 = particle.lat + v3*dt
    dep3 = particle.depth + w3*dt
    (u4, v4) = fieldset.UV[time + dt, dep3, lat3, lon3]
    w4 = fieldset.W[time + dt, dep3, lat3, lon3]
    particle.lon += (u1 + 2*u2 + 2*u3 + u4) / 6. * dt
    particle.lat += (v1 + 2*v2 + 2*v3 + v4) / 6. * dt
...
def AdvectionRK45(particle, fieldset, time, dt):
    ...
    (u6, v6) = fieldset.UV[time + c[4] * dt, particle.depth, lat5, lon5]
